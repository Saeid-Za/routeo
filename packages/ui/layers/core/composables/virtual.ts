...
import { useElementSize, useThrottleFn, watchDebounced } from "@vueuse/core"
...
export interface UseVirtualListOptionsBase {
	...
	/**
	 * The delay for Recalculate function
	 *
	 * @default 100 ms
	 */
 ...
 */
export function useVirtualList<T = any>(list: MaybeRefOrGetter<readonly T[]>, options: UseVirtualListOptions): UseVirtualListReturn<T> {
	...
	const calculateRangeInternal = useThrottleFn(calculateRange, options.throttleDelay ?? 100)
...
type UseVirtualListSource<T> = MaybeRefOrGetter<Readonly<T[]>>
...
function useVirtualListResources<T>(list: MaybeRefOrGetter<readonly T[]>): UseVirtualListResources<T> {
	...
	return { state, source: list, currentList, size, containerRef }
}
...
function createGetViewCapacity<T>(state: UseVirtualListResources<T>["state"], source: UseVirtualListResources<T>["source"], itemSize: UseVirtualListItemSize) {
	return (containerSize: number) => {
		...
		let sum = 0
		let capacity = 0
		for (let i = start; i < sourceValue.length; i++) {
...
function createCalculateRange<T>(type: "horizontal" | "vertical", overscan: number, getOffset: ReturnType<typeof createGetOffset>, getViewCapacity: ReturnType<typeof createGetViewCapacity>, { containerRef, state, currentList, source }: UseVirtualListResources<T>) {
	return () => {
		...
		if (element) {
			...
			state.value = {
				start: from < 0 ? 0 : from,
				end: to > sourceValue.length
					? sourceValue.length
					: to,
			}
			currentList.value = sourceValue
...
function useWatchForSizes<T>(size: UseVirtualElementSizes, list: MaybeRefOrGetter<readonly T[]>, containerRef: Ref<HTMLElement | null>, calculateRange: () => void) {
	watch([containerRef], () => {
		calculateRange()
	})

	watchDebounced([size.height, size.width], () => {
		calculateRange()
	}, { debounce: 100 })

	// Since list might be reactive or getter function, it must be watched deeply.
	watch(list, () => {
		calculateRange()
	}, { deep: true })
}
...
function createComputedTotalSize<T>(itemSize: UseVirtualListItemSize, source: UseVirtualListResources<T>["source"]) {
	return computed(() => {
		...
		if (typeof itemSize === "number")
			return sourceValue.length * itemSize

		return sourceValue.reduce((sum, _, index) => sum + itemSize(index), 0)
	})
}
...
function useHorizontalVirtualList<T>(options: UseHorizontalVirtualListOptions, list: MaybeRefOrGetter<readonly T[]>) {
...
function useVerticalVirtualList<T>(options: UseVerticalVirtualListOptions, list: MaybeRefOrGetter<readonly T[]>) {
	const resources = useVirtualListResources(list)
